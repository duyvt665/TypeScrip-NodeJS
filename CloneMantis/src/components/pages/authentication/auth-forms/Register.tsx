...
const Register = () => {
  ...
  const registerUserMutation = useMutation((data: FormData) => {
    return fetch("http://localhost:3000/authen/api/users", {
      method: "POST",
      headers: {
  ...
  if (registerUserMutation.isError) {
    console.error(registerUserMutation.error);
  ...
  return (
    <>
      ...
      <form onSubmit={onSubmit}>
        <div className="flex border-2 flex-col w-[480px] m-auto mt-[20px] p-10 bg-white rounded-lg drop-shadow-xl brightness-100">
          <div className="flex">
            <p className="font-semibold text-2xl">Sign up</p>
            <button
              onClick={handleLogin}
              className="text-sm pt-[10px] pl-[155px] text-blue-500"
            >
          ...
          <div className="flex gap-6 mt-6">
            <div className="block">
              <Label className="pt-8 !font-normal">First Name*</Label>
              <Input
            ...
            <div className="block">
              <Label className="pt-8 !font-normal">Last Name</Label>
              <Input
                className={`mt-2 ${errors.firstname ? "border-red-500" : ""}`}
                type="text"
                placeholder="Doe"
          ...
          <Label className="pt-8 !font-normal">Company</Label>
          <Input
            className="mt-2"
            type="text"
          ...
          <div className="flex mt-3">
            <div
              className={`bg-red-500 rounded-lg mt-3 h-2 pt-2 w-[60px] ${passwordValue.length >= 10 ? "bg-green-600 w-[398px]" : passwordValue.length >= 5 ? "!bg-yellow-500 w-[200px]" : ""}`}
            ></div>
            <div className="block ml-3 mt-1">
              {passwordValue.length >= 10 && (
                <p className="text-[15px]">Good</p>
              )}
              {passwordValue.length >= 5 && passwordValue.length < 10 && (
                <p className="text-[15px]">Normal</p>
              )}
              {passwordValue.length < 5 && <p className="text-[15px]">Weak</p>}
            </div>
          </div>

          <Label className="pt-8 !font-normal mb-2">
            Upload personal information
          </Label>
          <FileUpload></FileUpload>

          <p className="text-[13.6px] m-auto mt-10">
            By Signing up, you agree to our{" "}
            <a className="text-[#1677FF] hover:underline" href="">
              Term of service
            </a>{" "}
            and <a className="text-[#1677FF] hover:underline">Privacy Policy</a>
          </p>

          <Button
            type="submit"
            className="!mt-8 !bg-[#1677FF] !capitalize !h-[45px]"
            variant="contained"
          >
            Create Account
          </Button>
          ...
          <div className="flex items-center my-2 mt-7">
            ...
            <span className="px-3 text-xs">Sign up with</span>
            <hr className="flex-grow border-t border-[#f0f0f0]" />
          </div>
